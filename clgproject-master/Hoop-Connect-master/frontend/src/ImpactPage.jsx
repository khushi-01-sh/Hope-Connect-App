...
const ImpactPage = () => {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: livesImpacted,
      icon: <Smile size={24} />,
      sub: "Children supported",
      color: "#f59e0b",
      bg: "rgba(245,158,11,0.08)",
    },
    {
      label: "Items Distributed",
      value: itemsDistributed.toLocaleString("en-IN"),
      icon: <Package size={24} />,
      sub: "Clothes, Books, Food kits",
      color: "#22c55e",
    ...
    if (active && payload && payload.length) {
      return (
        ...
        >
          ...
          <p className="mb-0" style={{ color: "var(--hc-primary-light)" }}>
  ...
  useEffect(() => {
    const fetchDonationCategories = async () => {
      try {
        const res = await fetch("http://localhost:3200/donatedata");
        ...
        // Add demo donations from localStorage
        const demoDonations = JSON.parse(
          localStorage.getItem("demoDonations") || "[]",
        );
        demoDonations.forEach((d) => {
          const category = d.selectedType;
          if (allowedCategories.includes(category)) {
            const existing = finalData.find((item) => item.name === category);
            if (existing) {
              existing.value += 1;
            } else {
              finalData.push({ name: category, value: 1 });
            }
          } else {
            otherTotal += 1;
          }
        ...
        allowedCategories.forEach((cat) => {
          if (!finalData.find((d) => d.name === cat))
            finalData.push({ name: cat, value: 0 });
        });

        if (otherTotal > 0)
          finalData.push({ name: "Other Supplies", value: otherTotal });

        // If all values are 0, use dummy data
        const hasRealData = finalData.some((d) => d.value > 0);
        setDonationCategorie(
          hasRealData ? finalData : DUMMY_DONATION_CATEGORIES,
        );
      } catch (err) {
        console.log("Using dummy categories:", err.message);
        // Try to use demo donations even if backend fails
        const demoDonations = JSON.parse(
          localStorage.getItem("demoDonations") || "[]",
        );
        if (demoDonations.length > 0) {
          const allowedCategories = [
            "Food",
            "Education",
            "Clothes",
  ...
  useEffect(() => {
    const fetchDonations = async () => {
      try {
        const res = await fetch("http://localhost:3200/recent-donations");
        if (!res.ok) throw new Error("Server error");
        const data = await res.json();

        // Get demo donations from localStorage if in demo mode
        const demoDonations = JSON.parse(
          localStorage.getItem("demoDonations") || "[]",
        );

        // Format demo donations to match expected structure
        const formattedDemoDonations = demoDonations.map((d) => ({
          name: d.donorName || "Anonymous",
          quantity: d.description || d.selectedType,
          quan: d.quantity || "1",
          donationtype: d.selectedType,
          createdAt: d.createdAt,
        }));

        // Combine real and demo donations
        const allDonations = [...formattedDemoDonations, ...(data || [])];
        ...
        console.log("Using fallback donations:", err.message);
        // Try to get demo donations even if backend fails
        const demoDonations = JSON.parse(
          localStorage.getItem("demoDonations") || "[]",
        );
        const formattedDemoDonations = demoDonations.map((d) => ({
        ...
        if (formattedDemoDonations.length > 0) {
          setDonations(formattedDemoDonations);
        } else {
          setDonations(DUMMY_DONATIONS);
        }
      }
      ...
      <div style={{ background: "var(--hc-surface-alt)", minHeight: "100vh" }}>
        <div className="container py-5">
          ...
          <div className="row g-4 mb-5">
            ...
            <div
              className="col-lg-5 animate-fade-in-up"
          ...
          >
            ...
            >
              ...
              <h3
                ...
                style={{ fontFamily: "var(--hc-font-heading)" }}
              >
                A Story of Hope
              </h3>
              <p
              ...
              <div
                style={{ color: "var(--hc-primary-light)", fontWeight: 700 }}
              >
                — {DUMMY_TESTIMONIALS[activeTestimonial].author},{" "}
                {DUMMY_TESTIMONIALS[activeTestimonial].role}
              </div>
              {/* Dots */}
